...
所有生成规则由 LLM prompt 控制，直接编辑对应文件即可，无需改代码。

### 例句

编辑 `skills/generate_exercise/prompts/sentence.md`：

| 想要的效果 | 修改方式 |
|-----------|---------|
| 例句更长/更短 | 在「要求」中添加：`例句不超过 12 个单词` 或 `使用 15-25 个单词的长句` |
| 降低难度 | 添加：`使用初中词汇，句子结构简单` |
| 提高难度 | 添加：`使用学术英语，句子包含从句` |
| 特定场景 | 添加：`例句尽量使用校园/商务/日常生活场景` |
| 显示音标 | 添加：`并在 sentence 字段旁增加 phonetic 字段输出音标` |

### 选词填空

编辑 `skills/generate_exercise/prompts/cloze.md`：

| 想要的效果 | 修改方式 |
|-----------|---------|
| 降低难度 | 添加：`句子中保留该单词的首字母作为提示` |
| 提高难度 | 添加：`每道题增加一个干扰选项，输出 options 字段` |
| 改变题量 | 修改 `{count}` 参数为固定数字，如 `生成 5 道题` |
| 更详细的提示 | 修改 hint 说明：`hint 改为 8-15 个字的中文句子翻译` |
| 不随机乱序 | 去掉脚本中的 `random.shuffle(questions)` |

### Agent 行为

编辑 `prompts/agent_system.md`：

| 想要的效果 | 修改方式 |
|-----------|---------|
| 每批处理更多单词 | 添加：`generate_exercise 每批处理 15 个单词` |
| 例句生成后自检 | 添加：`例句生成后需检查质量，不合格则重新生成` |

### 分组大小

编辑 `lib/config.py`，修改 `GROUP_SIZE = 10` 为想要的数量。
